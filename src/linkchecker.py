...
def insert_or_update_link(conn, urlname, url, valid, result, info, warning, is_valid):
    with conn.cursor() as cur:
        # Get current status
        ...
        if existing_link:
            ...
            cur.execute("""
                ...
                WHERE id_link = %s
            """,(valid, result, info, warning, deprecated, consecutive_failures, link_id))
        else:
            # Insert new link (not deprecated on the first insertion)
            cur.execute("""
                INSERT INTO links (urlname, url, valid, result, info, warning, deprecated, consecutive_failures)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                RETURNING id_link
            """, (urlname, url, valid, result, info, warning, False, 0 if is_valid else 1))
            
            link_id = cur.fetchone()[0]
            
        # Insert new record in validation history
        cur.execute("""
            INSERT INTO validation_history(fk_link, validation_result)
            VALUES(%s, %s)
        """,(link_id, valid))
...
def main():
    ...
    # Specify the fields to include in the CSV file
    fields_to_include = ['urlname', 'parentname', 'baseref', 'valid', 'result', 'warning', 'info', 'url']
